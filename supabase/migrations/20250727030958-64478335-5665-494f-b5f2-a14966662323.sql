-- Critical Security Fix: Enable RLS on all public tables (excluding views)
...
-- Create user roles table for proper role management (if not exists)
DO $$ 
BEGIN
...
DROP POLICY IF EXISTS "Authenticated users only" ON public.client_stats;
CREATE POLICY "Authenticated users only" ON public.client_stats
    FOR ALL TO authenticated USING (true);

DROP POLICY IF EXISTS "Authenticated users only" ON public.conversation_daily_data;
CREATE POLICY "Authenticated users only" ON public.conversation_daily_data
    FOR ALL TO authenticated USING (true);

DROP POLICY IF EXISTS "Authenticated users only" ON public.conversation_metrics;
CREATE POLICY "Authenticated users only" ON public.conversation_metrics
    FOR ALL TO authenticated USING (true);

DROP POLICY IF EXISTS "Authenticated users only" ON public.conversion_by_time;
CREATE POLICY "Authenticated users only" ON public.conversion_by_time
    FOR ALL TO authenticated USING (true);

DROP POLICY IF EXISTS "Authenticated users only" ON public.dashboard_metrics;
CREATE POLICY "Authenticated users only" ON public.dashboard_metrics
    FOR ALL TO authenticated USING (true);

-- Restrict access to sensitive audit and backup tables
DROP POLICY IF EXISTS "Admins only access" ON public.audit_log;
CREATE POLICY "Admins only access" ON public.audit_log
    FOR ALL USING (public.is_admin_user());

DROP POLICY IF EXISTS "Admins only access" ON public.contact_stage_history_backup;
CREATE POLICY "Admins only access" ON public.contact_stage_history_backup
    FOR ALL USING (public.is_admin_user());

DROP POLICY IF EXISTS "Admins only access" ON public.contacts_backup;
CREATE POLICY "Admins only access" ON public.contacts_backup
    FOR ALL USING (public.is_admin_user());

DROP POLICY IF EXISTS "Admins only access" ON public.custom_field_audit_log;
CREATE POLICY "Admins only access" ON public.custom_field_audit_log
    FOR ALL USING (public.is_admin_user());

-- AI products should be publicly readable
DROP POLICY IF EXISTS "Public read access" ON public.ai_products;
CREATE POLICY "Public read access" ON public.ai_products
    FOR SELECT USING (true);

DROP POLICY IF EXISTS "Admins can manage AI products" ON public.ai_products;
...
DROP POLICY IF EXISTS "Authenticated users only" ON public.campaign_data;
CREATE POLICY "Authenticated users only" ON public.campaign_data
    FOR ALL TO authenticated USING (true);

DROP POLICY IF EXISTS "Authenticated users only" ON public.campaign_recipients;
CREATE POLICY "Authenticated users only" ON public.campaign_recipients
    FOR ALL TO authenticated USING (true);

DROP POLICY IF EXISTS "Authenticated users only" ON public.campaigns;
CREATE POLICY "Authenticated users only" ON public.campaigns
    FOR ALL TO authenticated USING (true);

DROP POLICY IF EXISTS "Authenticated users only" ON public.calendar_attendees;
CREATE POLICY "Authenticated users only" ON public.calendar_attendees
    FOR ALL TO authenticated USING (true);

DROP POLICY IF EXISTS "Authenticated users only" ON public.coupon_redemptions;
...
DROP POLICY IF EXISTS "Authenticated users only" ON public.client_custom_values;
CREATE POLICY "Authenticated users only" ON public.client_custom_values
    FOR ALL TO authenticated USING (true);

DROP POLICY IF EXISTS "Authenticated users only" ON public.custom_field_validation_rules;
